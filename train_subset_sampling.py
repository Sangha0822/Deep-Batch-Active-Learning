...
if __name__ == "__main__":
    ...
    results_file = open("results_subset_sampling.txt", "a")
    ...
    for i in range(QUERY_ROUNDS):
        ...
        # --- New Optimized Querying Logic ---
        # Idea: Checking the tradeoffs between reducing the time and the accuracy. 
        # Making the initial random step to be small enough to be solved quickly.

        # Randomly sample a smaller "candidate set" from the large unlabeled pool
        num_candidates = min(CANDIDATE_SET_SIZE, len(unlabeled_indices))
        candidate_indices = np.random.choice(unlabeled_indices, size=num_candidates, replace=False)

        unlabeled_subset = Subset(train_dataset, candidate_indices)
        unlabeled_dataloader = DataLoader(dataset=unlabeled_subset,
                                        batch_size=1,
                                        shuffle=False)

        print(f"Running BADGE on a candidate set of {len(candidate_indices)} samples...")
        ...
        # 4. Map the selected indices back to their original values from the candidate set
        queries_indices = [candidate_indices[i] for i in relative_indices_to_query]

        end_time = time.time()
        query_time = end_time - start_time

        queried_set = set(queries_indices) # added set to remove duplicates
